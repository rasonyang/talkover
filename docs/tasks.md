...
### T0.4 CI for GPU-free tests — `doing`

- 2026-09-17: workflow written and validated locally (YAML parse, same lint / test commands green). Acceptance is open until the repository is pushed and a run on `main` is green. Open risks: CUDA torch wheel size on the runner; upstream packages building on Linux + Python 3.12.
...
### T3.2 `BrainLLM` protocol with Anthropic and OpenAI-compatible clients — `doing`
