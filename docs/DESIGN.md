...
`talkover.app.build_app` is the one place the three meet, and `talkover serve` is nothing but "load the config, apply the patches for the configured device, build these objects, run uvicorn". The engine is started and stopped by the application's ASGI lifespan, never by the protocol layer. ASR is a **tap on `feed_pcm16`**: the 1 s units of 16 kHz pcm16 the model is fed are exactly what the ASR backend wants, so they go into a bounded queue beside the model rather than down a second audio path, and every resulting event is fanned out to both consumers — the Realtime session (turn detection and transcripts, 5.4) and the Brain bridge (trusted text, 6.2). Dropping a transcript when ASR falls behind is the intended failure; blocking the audio path into the model is not.
...
**token2wav on MPS (T1.5, M3 Max)**: the vocoder alone costs 0.49 s per unit at upstream's `n_timesteps=10`, 0.24 s at 5 and 0.14 s at 2, all float32 — the upstream default already exceeds the 0.35 s budgeted for Talker plus token2wav, with the AR stage excluded. `float16=True` aborts the process on MPS (MPSGraph dtype mismatch), so the vocoder stays float32 there. The flow-matching step count is the knob: `engine.token2wav_timesteps` is **implemented** (section 9) and reaches `build_token2wav` through `talker_factory_from_config`. It still ships at upstream's 10; which value to run is T1.9's decision after a listening check, because only the latency of 5 and 2 has been measured, not their quality.

**What `bench_rtf.py` measures (T1.8)**: the stages above are budget lines, not measurement points; upstream reports a different set, so the tool reports what exists. `unit` (and `flush`) is the wall time of one `feed_pcm16` round trip on the event loop — the < 1.0 s line; `step` is the same unit measured on the inference thread with `backend.synchronize()` on both sides, so the difference between the two is the thread hand-over. `thinker`, `talker_prep`, `talker` and `token2wav` come from the upstream step metrics (`cost_llm`, `cost_tts_prep`, `cost_tts`, `cost_token2wav`) through `EngineSession`'s optional `on_unit_timing` hook, and `asr` is timed by the bench harness itself, because ASR is a side channel the engine never runs (4.3). There is no separate `audio_encode` row: upstream folds encoding the input audio into `cost_llm`, so the first table row is the whole Thinker step the 0.45 s line budgets. The same hook carries the Thinker token ids per unit, which is what the JSON dump diffs across backends; speak-token ids are not exposed by upstream's step event, so the dump carries their count (`n_tts_tokens`) instead.
...
| `EngineStepEvent(is_audio_chunk=True)` | `response.output_audio.delta` for the unit its `unit_id` / `generation_id` name. With the Talker thread (4.2) the waveform is vocoded after the Thinker step that produced it has already been reported, so it arrives on its own event; it may only extend a response that is still open. A chunk with no response to extend is dropped and counted in `ResponseMapper.dropped_audio_chunks` |
| `EngineStepEvent(talker_done=True)` | Nothing on its own. It is the Talker's drained marker for the turn its `generation_id` names — `EngineSession` publishes it when the Talker finishes the request that carried `end_of_turn`, and when a generation is cancelled — and it is what releases a held close chain (next row) |
| `end_of_turn: true` | `response.output_audio.done`, `transcript.done`, `content_part.done`, `output_item.done`, `response.done` with `status: "completed"`. With the Talker thread (4.2) the chain is **held**: the tail of the turn has not been vocoded yet, so the response is marked pending-close and the chain is emitted only when the `talker_done` marker for that `generation_id` arrives. Chunks arriving inside the hold are delivered normally; a non-listen unit arriving inside it belongs to the next turn and flushes the hold first. `mapping.TALKER_DRAIN_TIMEOUT_SEC` (3 s) bounds the wait, so a Talker failure costs the tail of one turn and never the response. The in-step Talker is detected by `metrics["talker"]["mode"]` and is never held |
| `interrupted: true` | The same close chain with `status: "cancelled"`, emitted immediately and never held — a barge-in makes whatever is still in the Talker stale: the assistant item is marked `incomplete`, `conversation.item.truncated` reports the audio already sent, and `response.done.status_details` is `{"type": "cancelled", "reason": ...}` — `client_cancelled` after a `response.cancel`, `turn_detected` for a model barge-in. `response.usage` is best-effort on every close: a character heuristic for the text tokens, audio tokens always 0 |
...
- When the window closes, `SessionSlot` closes the runner and awaits the `on_release` hook of `create_app`; it never calls `engine.stop()` itself. `talkover.app` wires that hook to a **reset**: it closes the Brain bridge, unbinds and resets the ASR stream, and returns the engine to an idle state without unloading it, so `/health` reports `idle` again and the process serves the next call. Upstream `DuplexLiveSession` has no "start a new conversation" call short of rebuilding the session — which would reload the weights — so the reset is best effort and layered: an engine exposing `reset()` is asked for one, any other engine is told to `interrupt_output()`, and a failure is logged rather than raised. Starting and stopping the engine belongs to the application's lifespan instead (section 3). An expired or unknown `session_id` is not an error: it is ignored and a fresh session is created. A session that ended with a fatal error is released at once, with no window.
...
engine:
  ...
  token2wav_timesteps: 10    # vocoder flow-matching steps, 1..50 (4.4)
...
`engine.token2wav_timesteps` is the vocoder's flow-matching step count (4.4), an integer in `1..50` defaulting to upstream's 10. It is passed to `stepaudio2.Token2wav` by `build_token2wav`, and `talkover check` echoes it on the `engine.token2wav_dir` line. Lowering it is the only latency knob the vocoder has; which value to ship is T1.9's decision, because only the cost of 5 and 2 has been measured, not their quality.
...
| With the Talker thread (4.2), the tail of a turn is vocoded after `end_of_turn` has already been reported | The tail of an assistant turn could be missing from the client's audio | **Settled (2026-09-18).** The `end_of_turn` close chain is held until the Talker's `talker_done` marker for that `generation_id` arrives, and chunks inside the hold are delivered normally (5.3). `EngineSession` emits the marker from `TalkerDone(end_of_turn)` and from `TalkerInterrupted`; `mapping.TALKER_DRAIN_TIMEOUT_SEC` (3 s) bounds the wait so a Talker failure costs the tail of one turn and never the response. An interrupt is never held. The in-step Talker is unaffected, and is recognised by `metrics["talker"]["mode"]` rather than guessed |
